...
| `03:00` Sunday | `backup-object-lock-renew.py` | Walks still-present `library/` objects and pushes their `retain-until-date` to now +90 d (sliding retention), boto3 with a 16-thread pool. `db-backups/` is deliberately **not** renewed — those age out via the bucket lifecycle. |
